...
bool r7_game_play_card(R7Game *rg) {
	...
	const Card *min_card = stack_read_first(rg->build[build_index]);
	const Card *max_card = stack_read_bottom(rg->build[build_index]);

	uint8_t min_value = card_value(min_card);
	uint8_t max_value = card_value(max_card);

	if (card_to_play_value == max_value + 1) {
		stack_pop_from_top_to_top(rg->deck, rg->build[build_index], true);
	} else if (card_to_play_value == min_value - 1) {
		stack_pop_from_top_to_bottom(rg->deck, rg->build[build_index], true);
	} else {
		stack_pop_from_top_to_top(rg->deck, rg->bin, false);
	}
	return true;
}
